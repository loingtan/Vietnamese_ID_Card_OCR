"""
Vietnamese ID Card OCR Application - Refactored Version

This file serves as the main entry point for the Streamlit web application.
The original monolithic code has been refactored into a modular structure.

For the original code (1497 lines), see app_backup.py
"""

# Import the refactored Streamlit UI
...
def process_image_with_gemini(image):
    ...
    Returns:
        dict: Structured information extracted from the ID card
    """
    try:
...
def detect_id_card(image, models, device, expand_ratio=0.1):
    ...
    if len(corners) == 3:
        ...
        if len(new_corners) == 4:
            ...
            st.error(
                f"Failed to calculate fourth corner. Got {len(new_corners)} corners.")
    ...
    if len(corners) >= 4:
        ...
        if rotation_angle != 0:
            st.write(
                f"Image rotated by {rotation_angle} degrees based on QR code position")
            return final_id
...
def process_image(image):
    ...
    info = process_image_with_gemini(pil_image)
    return {
